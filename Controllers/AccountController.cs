        ...
        public ActionResult Validate(Admins admin)
        {
            ...
            if (_admin != null)
            {
                if (_admin.Password == admin.Password)
                {
                    ...
                    HttpContext.Session.SetString("level", _admin.Level);

                    int roleId = (int)HttpContext.Session.GetInt32("role_id");

                    //var _role = db.Roles.Where(s => s.Id == roleId).FirstOrDefault();
                    ...
                    HttpContext.Session.SetString("usernames", _admin.Email);
